...
class Message
{
    ...
    public function addMessage($groups, $destinatario) {

        $destinatario = $destinatario == 0 ? "NULL" : $destinatario;
        $this->data_fine = substr($this->data_fine, 8, 2) . "/" . substr($this->data_fine, 5, 2) .
            "/" . substr($this->data_fine, 0, 4);
        if ($this->data_fine == "//") $this->data_fine = "";
        $sql = "INSERT INTO messaggi (tipo, testo, id_autore, titolo, data_fine, id_destinatario) VALUES ('" .
                $this->db->escape_string($this->tipo)."', '" . $this->db->escape_string($this->testo) . "', " .
                $this->db->escape_string($this->id_autore).", '" . $this->db->escape_string($this->titolo) . "', '" .
                $this->data_fine . "', " . $destinatario . ")";
        ...
        if ($result) {
            ...
            return "<div class='panel panel-danger'><div class='panel-heading'>" . $sql . " Problema nell'invio del messaggio. Se il problema persiste contattare l'amministratore.</div></div>";
        }
        ...
        foreach ($groups as $value) {
            $comm = "NULL";
            if(strstr($value,"comm_")){
              $comm = explode("_", $value);
              $comm = $comm[1];
              $value = "NULL";
            }
            $count++;
            $sql = "INSERT INTO message_groups (id_messaggio, id_gruppo, id_destinatario, id_commessa) VALUES (" .
                $insert_id . ", " . $value . ", " . $destinatario . ", ". $comm .")";
            $this->db->query($sql);

            $sql = "INSERT INTO file_groups (id_messaggio, id_gruppo, id_destinatario, id_commessa) VALUES (" .
                $insert_id . ", " . $value . ", " . $destinatario . ", ". $comm .")";
            $this->db->query($sql);

            if ($comm != ""){
              $sql_group = "SELECT a.user_id FROM user_commesse as ug, anagrafica a WHERE ug.id_user = a.user_id AND ug.id_user != '" .
                      $destinatario . "' AND ug.id_commessa = " . $comm;
            } else {
              $sql_group = "SELECT a.user_id FROM user_groups as ug, anagrafica a WHERE ug.id_user = a.user_id AND ug.id_user != '" .
                      $destinatario . "' AND ug.id_gruppo = " . $value;
            }

            $result_group = $this->db->query($sql_group);

            while ($obj_group = $result_group->fetch_object()) {
                $sql_read = "INSERT INTO read_messages (id_messaggio, id_user) VALUES (" . $insert_id . ", " . $obj_group->user_id . ")";
                $this->db->query($sql_read);
            }
        }

        if ($destinatario != 0) {
            if ($count == 0) {
                $sql = "INSERT INTO message_groups (id_messaggio, id_gruppo, id_destinatario, id_commessa) VALUES (" . $insert_id .
                    ", NULL, " . $destinatario . ", NULL)";
                $this->db->query($sql);

                $sql = "INSERT INTO file_groups (id_messaggio, id_gruppo, id_destinatario, id_commessa) VALUES (" . $insert_id .
                    ", NULL, " . $destinatario . ", NULL)";
                $this->db->query($sql);
            }
            $sql = "INSERT INTO read_messages (id_messaggio, id_user) VALUES (" . $insert_id . ", " . $destinatario . ")";
            $this->db->query($sql);
        }
        return $alert;
    }
...
if((!is_dir($file))&($file!=".")&($file!=".."))
   ...
   $result=$this->db->query($sql);
    ...
    public function updateMessage($id){
         ...
         $sql="UPDATE messaggi SET testo = '".$this->db->escape_string($this->testo)."', titolo= '".$this->db->escape_string($this->titolo)."' WHERE id_messaggio =".$id;
