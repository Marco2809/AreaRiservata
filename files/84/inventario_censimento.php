...
if ($action == "Aggiorna")
{
    $array_asset = !empty($_GET['dati_asset']) ? $_GET['dati_asset'] : NULL;
    if (!empty($array_asset))
    {
        foreach ($array_asset as $key_cod_asset => $dati_asset)
        {
            $codice_asset = !empty($_GET['dati_asset'][$key_cod_asset]['cod_asset']) ? $_GET['dati_asset'][$key_cod_asset]['cod_asset'] : null;
            $codice_famiglia = !empty($_GET['dati_asset'][$key_cod_asset]['cod_famiglia']) ? $_GET['dati_asset'][$key_cod_asset]['cod_famiglia'] : null;

            $corridoio = !empty($_GET['dati_asset'][$key_cod_asset]['corridoio']) ? $_GET['dati_asset'][$key_cod_asset]['corridoio'] : null;
            $scaffali = !empty($_GET['dati_asset'][$key_cod_asset]['scaffali']) ? $_GET['dati_asset'][$key_cod_asset]['scaffali'] : null;
            $ripiano = !empty($_GET['dati_asset'][$key_cod_asset]['ripiano']) ? $_GET['dati_asset'][$key_cod_asset]['ripiano'] : null;
            $pt_number = !empty($_GET['dati_asset'][$key_cod_asset]['pt_number']) ? $_GET['dati_asset'][$key_cod_asset]['pt_number'] : null;
            $serial_number = !empty($_GET['dati_asset'][$key_cod_asset]['serial_number']) ? $_GET['dati_asset'][$key_cod_asset]['serial_number'] : null;
            $imei_number = !empty($_GET['dati_asset'][$key_cod_asset]['imei_number']) ? $_GET['dati_asset'][$key_cod_asset]['imei_number'] : null;
            $n_asset_censire = !empty($_GET['dati_asset'][$key_cod_asset]['n_asset_censire']) ? $_GET['dati_asset'][$key_cod_asset]['n_asset_censire'] : null;

            $ddt = !empty($_GET['dati_asset'][$key_cod_asset]['num_ddt1']) ? $_GET['dati_asset'][$key_cod_asset]['num_ddt1'] : null; // separato
            $data = !empty($_GET['dati_asset'][$key_cod_asset]['data_ddt1']) ? $_GET['dati_asset'][$key_cod_asset]['data_ddt1'] : null;
            $fattura = !empty($_GET['dati_asset'][$key_cod_asset]['fattura']) ? $_GET['dati_asset'][$key_cod_asset]['fattura'] : null;
            $data_fattura = !empty($_GET['dati_asset'][$key_cod_asset]['data_fattura']) ? $_GET['dati_asset'][$key_cod_asset]['data_fattura'] : null;


            $sql = "UPDATE " . MAIN_DB_PREFIX . "asset SET "
                    . "corridoio =" . "'" . $db->escape($corridoio) . "'"
                    . ",scaffali=" . "'" . $db->escape($scaffali) . "'"
                    . ",ripiano=" . "'" . $db->escape($ripiano) . "'"
                    . ",pt_number=" . "'" . $db->escape($pt_number) . "'"
                    . ",serial_number=" . "'" . $db->escape($serial_number) . "'"
                    . ",imei_number=" . "'" . $db->escape($imei_number) . "'"
                    . " WHERE  cod_asset LIKE " . "'" . $db->escape($codice_asset) . "'";
            $res = $db->query($sql);
            if ($res) //se l'aggiornamentio è stato eseguito
            {
                $flag = 1; // 1 aggiornato 0 non aggiornato
                $flag = controlla_flag($corridoio, $scaffali, $ripiano, $pt_number, $serial_number, $imei_number);
                if ($flag == 1)
                {
                    $query = "UPDATE " . "tmp_inventario_censimento SET "
                            . "flag_aggiornato =" . "'" . $flag . "'"
                            . " WHERE  codice_asset LIKE " . "'" . $db->escape($codice_asset) . "'";
...
if ($action != "Aggiorna")
{
    ...
    if ($action == "Crea") // se ha inserito il pulsante crea, non mostrare più il bottone
    {
        ...
        print '<input type="submit" class="button" name="action"  formtarget="_blank" value="' . "Stampa tutti Qrcode" . '"> '; //mostra solo gli asset da creare
...
{
    print '<center><input type="submit" class="button" name="action"  value="' . "Termina Procedura" . '"> ';
    print '<input type="submit" class="button" name="action"  formtarget="_blank" value="' . "Stampa tutti Qrcode" . '"> </center>'; //mostra solo gli asset da creare
}
print '</form>';
...
if ($action == "Stampa tutti Qrcode")
{
    $codice_famiglia = !empty($_GET['cod_famiglia']) ? $_GET['cod_famiglia'] : null;
    $path = DOL_URL_ROOT . '/product/qrcode_massivo.php?cod_famiglia=' . $codice_famiglia;
    print '<META HTTP-EQUIV="Refresh" CONTENT="0; url=' . $path . '">';
}
...
function visualizza_asset_creati($codice_famiglia)
{
    ...
    if ($res)
    {
        while ($obj_asset = $db->fetch_object($res))
        {
            ...
            $aggiornamento_asset = "";
            if (empty($obj_asset->flag_aggiornato))
                $aggiornamento_asset = "Asset non aggiornato";
            else
                $aggiornamento_asset = "Asset  aggiornato";

            print '<tr>';
            print '<td width="15%">Aggiornamento asset</td>';
            print '<td class="nobordernopadding">' . $aggiornamento_asset . '</td>';
            print '</tr>';
            ...
            print '<td><input name="dati_asset[' . $codice_asset . '][corridoio]" size="20" value="' . $corridoio . '">';
            print '</td>';
            print '</tr>';

            print '<tr>';
            print '<td width="15%">Scaffali</td>';
            print '<td><input name="dati_asset[' . $codice_asset . '][scaffali]" size="20" value="' . $scaffali . '">';
            print '</td>';
            print '</tr>';

            print '<tr>';
            print '<td width="15%">Ripiano</td>';
            print '<td><input name="dati_asset[' . $codice_asset . '][ripiano]" size="20" value="' . $ripiano . '">';
            print '</td>';
            print '</tr>';
            ...
            print '<td><input name="dati_asset[' . $codice_asset . '][pt_number]"  onkeyup="showHint(this.value)" size="20" value="' . $pt_number . '">';
            print '</td>';
            ...
            print '<td><input name="dati_asset[' . $codice_asset . '][serial_number]" size="20" value="' . $serial_number . '">';
            print '</td>';
            print '</tr>';
            ...
            print '<td><input name="dati_asset[' . $codice_asset . '][imei_number]" size="20" value="' . $imei_number . '">';
            print '</td>';
            print '</tr>';

            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][id_magazzino]" value="' . $obj_asset->id_magazzino . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][cod_famiglia]" value="' . $obj_asset->cod_famiglia . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][n_asset_censire]" value="' . $n_asset_censire . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][cod_asset]" value="' . $obj_asset->cod_asset . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][id_regione]" value="' . $obj_asset->id_regione . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][stato_tecnico]" value="' . $obj_asset->stato_tecnico . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][stato_fisico]" value="' . $obj_asset->stato_fisico . '"> ';

            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][num_ddt1]" value="' . $obj_asset->num_ddt1 . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][data_ddt1]" value="' . $obj_asset->data_ddt1 . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][fattura]" value="' . $obj_asset->fattura . '"> ';
            print ' <input type="hidden" name="dati_asset[' . $codice_asset . '][data_fattura]" value="' . $obj_asset->data_fattura . '"> ';
...
//controlla se sono stati settati i campi
function controlla_flag($corridoio, $scaffali, $ripiano, $pt_number, $serial_number, $imei_number)
{
...
function censisci($cod_famiglia, $n_asset_censire, $ddt, $data, $fattura, $data_fattura, $id_magazzino, $id_regione)
{
    ...
    $user_id = $user->id;
    $sql = "CREATE TABLE  tmp_inventario_censimento(id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, codice_asset VARCHAR(250),codice_famiglia VARCHAR(250),  id_user VARCHAR(100), flag_aggiornato VARCHAR(100) )";
    $result = $db->query($sql);
